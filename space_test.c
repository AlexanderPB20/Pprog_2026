...
int main(int argc, char** argv) {
  ...
  if (all || test == 14) test1_space_has_object();
  if (all || test == 15) test2_space_has_object();
  if (all || test == 16) test1_space_set_character();
  if (all || test == 17) test2_space_set_character();
  if (all || test == 18) test1_space_get_character();
  if (all || test == 19) test2_space_get_character();
  if (all || test == 20) test1_space_set_gdesc();
  if (all || test == 21) test2_space_set_gdesc();
...
void test2_space_del_object() {
  Space *s = space_create(1);
  PRINT_TEST_RESULT(space_del_object(s, 10) == ERROR); /* Borrar lo que no existe */
  space_destroy(s);
}
...
void test1_space_set_gdesc() {
  Space *s = space_create(1);
  PRINT_TEST_RESULT(space_set_gdesc(s, 0, "TestDesc") == OK);
  space_destroy(s);
}
void test2_space_set_gdesc() {
  Space *s = space_create(1);
  PRINT_TEST_RESULT(space_set_gdesc(s, 10, "TestDesc") == ERROR); /* Falla por línea fuera de rango */
  space_destroy(s);
}
