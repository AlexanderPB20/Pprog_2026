...
void game_actions_next(Game *game) {
  ...
  current_id = game_get_connection(game, current_id, S);
  if (current_id != NO_ID) {
...
void game_actions_take(Game *game) {
  ...
  for (i=0;i<game_get_n_objects(game);i++)
  {
    ...
    if ((game_get_object_location(game, object_id) == space_id) && player_get_object_id(player) == NO_ID) // si el objeto esta en el espacio y el jugador no tiene objeto
    {
      if (strcmp(object_input, object_get_name(obj)) == 0) // compara si el argumento y el nombre del objeto son iguales
      {
        player_set_object_id(player, object_get_id(obj)); // jugador toma objeto
...
void game_actions_drop(Game* game) {
  ...
  if (!space_has_object(game_get_space(game, space_id), object_id)) // player tiene objeto
  {
    ...
    player_set_object_id(player, NO_ID); // jugador deja el objeto
