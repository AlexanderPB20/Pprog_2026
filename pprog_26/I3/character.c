...
struct _Character {
    ...
    char name[WORD_SIZE + 1];   /* The name of the character */
    char gdesc[7];              /* The description of the character */
    int health;                 /* The health of the character */
    Bool friendly;              /* Whether the character is friendly or not */
    char message[WORD_SIZE + 1]; /* The message sent from the character */
};
...
Status character_print(Character *character) {
    ...
    fprintf(stdout, "Character (Id: %ld, Name: %s, Health: %d, Friendly: %s, Message: %s, Gdesc: %s)\n", character_get_id(character), character_get_name(character), character_get_health(character), character_is_friendly(character) ? "Yes" : "No", character_get_message(character), character_get_gdesc(character));
    return OK;
}
