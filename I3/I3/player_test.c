 ...
 */
 ...
 int main(int argc, char** argv) {
  ...
  if (all || test == 27) test1_player_set_max_objs();
  if (all || test == 28) test2_player_set_max_objs();
  if (all || test == 29) test3_player_set_max_objs();
  if (all || test == 30) test4_player_set_max_objs();
  if (all || test == 31) test1_player_remove_object_of_backpack();
  if (all || test == 32) test2_player_remove_object_of_backpack();
  if (all || test == 33) test3_player_remove_object_of_backpack();
  if (all || test == 34) test4_player_remove_object_of_backpack();
  if (all || test == 35) test1_player_has_object();
  if (all || test == 36) test2_player_has_object();
  if (all || test == 37) test3_player_has_object();
  if (all || test == 38) test4_player_has_object();
  if (all || test == 39) test1_player_get_backpack();
  if (all || test == 40) test2_player_get_backpack();
  if (all || test == 41) test1_player_set_backpack();
  if (all || test == 42) test2_player_set_backpack();
  if (all || test == 43) test3_player_set_backpack();
  if (all || test == 44) test1_player_backpack_is_full();
  if (all || test == 45) test2_player_backpack_is_full();
  if (all || test == 46) test3_player_backpack_is_full();
  if (all || test == 50) test1_player_get_object_from_backpack();
  if (all || test == 51) test2_player_get_object_from_backpack();
  if (all || test == 52) test3_player_get_object_from_backpack();
...
void test4_player_add_object_to_backpack() { /*obj repe*/
  Player* p = player_create(5);
...
void test5_player_add_object_to_backpack() { /*sin espacio*/
  Player* p = player_create(5);
  int i;
...
void test1_player_set_max_objs() {
  Player *p = player_create(5);
  PRINT_TEST_RESULT(player_set_max_objs(p, 10) == OK);
  player_destroy(p);
}

void test2_player_set_max_objs() {
  Player *p = NULL;
  PRINT_TEST_RESULT(player_set_max_objs(p, 10) == ERROR);
}

void test3_player_set_max_objs() {
  Player *p = player_create(5);
  PRINT_TEST_RESULT(player_set_max_objs(p, -1) == ERROR);
  player_destroy(p);
}

void test4_player_set_max_objs() {
  Player *p = player_create(5);
  PRINT_TEST_RESULT(player_set_max_objs(p, -1) == ERROR);
  player_destroy(p);
}
...
void test1_player_remove_object_of_backpack() {
  Player* p = player_create(5);
  player_add_object_to_backpack(p, 2);
  PRINT_TEST_RESULT(player_remove_object_of_backpack(p, 2)==OK);
  player_destroy(p);
}

void test2_player_remove_object_of_backpack() {
  Player* p = NULL;
  PRINT_TEST_RESULT(player_remove_object_of_backpack(p, 2)==ERROR);
}

void test3_player_remove_object_of_backpack() {
  Player* p = player_create(5);
  PRINT_TEST_RESULT(player_remove_object_of_backpack(p, NO_ID)==ERROR);
  player_destroy(p);
}
...
void test4_player_remove_object_of_backpack() { /*no that obj*/
  Player* p = player_create(5);
  player_add_object_to_backpack(p, 2);
  PRINT_TEST_RESULT(player_remove_object_of_backpack(p, 1)==ERROR);
  player_destroy(p);
}
...
void test2_player_set_health() {
  Player *p = player_create(5);
  PRINT_TEST_RESULT(player_set_health(p, -5) == ERROR); /* Vida negativa */
  player_destroy(p);
}
